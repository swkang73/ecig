...
def plot_calibration_curve(X_train, X_test, clf, name, fig_index, axis_1, axis_2):
    ...
    f.write("\tPrecision: %1.3f" % precision_score(y_test, y_pred, average='weighted'))
    f.write("\tRecall: %1.3f" % recall_score(y_test, y_pred, average='weighted'))
...
plot_calibration_curve(raw_X_train, raw_X_test, LogisticRegression(solver='lbfgs'), "LR", 1, ax1, ax2)

# plot calibration curve for SS + artificial neural network 
plot_calibration_curve(pca_X_train, pca_X_test, MLPClassifier(solver='lbfgs', alpha=1e-5, hidden_layer_sizes=(5, 2), random_state=RANDOM_STATE), "PCA + ANN", 2, ax1, ax2)
...
# Plot calibration curve for Gaussian naive bayes
plot_calibration_curve(raw_X_train, raw_X_test, GaussianNB(), "Gaussian NB", 6, ax1, ax2)
